...
class CrawlCctd():
    def __init__(self):
        self.log = Log().logger
        self.local = HttpUtil('http://torch.togeek.cn:8080/powerMobileApp/', 'admin', 'www.togeek.cn')
        self.city = {
    ...
    def crawl_yhlddc(self, start_date, end_date):
        '''
        爬取沿海六大电厂（日）的供耗存数据，post到手电后台系统，一天一次
        :param start_date: 开始日期
        :param end_date: 结束日期
        :return: None
        '''
    ...
    def start(self):
        ...
        begin = (today - datetime.timedelta(days=30)).strftime('%Y-%m-%d %H:%M:%S')  # 爬取一个月的数据
